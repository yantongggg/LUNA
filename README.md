...
CREATE POLICY "Users can delete their own incident reports"
  ON public.incident_reports FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view their own incident_reports"
  ON incident_reports FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own incident_reports"
  ON incident_reports FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own incident_reports"
  ON incident_reports FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own incident_reports"
  ON incident_reports FOR DELETE USING (auth.uid() = user_id);
...
supabase functions deploy analyze-evidence --debug
...
2. Check your Google AI Studio API key is valid
3. Ensure you haven't exceeded free tier rate limits (15 req/min for Gemini Flash)
...
2. Check Maps JavaScript API is enabled in Google Cloud Console
3. Verify API key has correct referer restrictions
4. Check browser console for specific error messages

#### Issue: "Voice not playing"

**Solution**:
1. Verify AZURE_TTS_API_KEY and AZURE_TTS_REGION are set in Edge Function secrets
2. Check Azure Speech Services resource is active
3. Ensure user has interacted with page (iOS/Safari audio unlock)
